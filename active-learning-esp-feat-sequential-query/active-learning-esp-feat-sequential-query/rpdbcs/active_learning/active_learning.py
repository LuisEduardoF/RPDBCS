...
def active_learning(active_estimator: ActiveLearner, Xpool, Ypool, Xtest,
                    Ytest, query_size: int, query_budget: int, metrics, hidden,
                    cm_list=None):
    """
    ...
    while(query_budget > 0):
        ...
        if hidden is not None:
            hidden_class_picked = (Ypool[query_idx] == hidden).sum()
            print(f'{hidden_class_picked} examples of hidden class {hidden} '
                  'were picked')
