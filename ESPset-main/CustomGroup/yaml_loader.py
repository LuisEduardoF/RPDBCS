...
from CustomGroupKFold import CustomGroupKFold
...
class Config:
    cross_validators = {'stratifiedkfold': sklearn.model_selection.StratifiedKFold,
                        ...
                        'groupkfold': CustomGroupKFold}

    param_mapping = {
        'sklearn.neighbors.KNeighborsClassifier': {'k': 'n_neighbors'}}
    ...
    def __init__(self, dataset_path, save_file, **kwargs):
        ...
        if (len(kwkeyset - attr_set) > 0):
            raise RuntimeError(YAML_INVALID_ERROR_MSG %
                               (kwkeyset-attr_set, set(_attrs)))
    ...
    def split_gridsearchparams(params):
        grid_search_params = {p: v for p,
                              v in params.items() if isinstance(v, list)}
    ...
    def getBaseClassifiers(self):
        ...
        for bclf in self.base_classifiers:
            ...
            clf_params = {parameters_abbreviation(
                clf_name, p): v for p, v in clf_params.items()}
            ...
            clf_params, gridsearch_params = Config.split_gridsearchparams(
                clf_params)
            #####################################
            ...
            if (scaler is not None):
                clf = sklearn.pipeline.Pipeline([('scaler', scaler),
                                                 ('clf', clf)])
                gridsearch_params = {'clf__%s' %
                                     p: v for p, v in gridsearch_params.items()}
