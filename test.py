...
def createGridSearch(clf, param_grid, n_jobs=None, gridsearch_constructor=GridSearchCV):
    ...
    if (has_gridsearch):
        gridsearch_sampler = StratifiedShuffleSplit(
            n_splits=1, test_size=0.2, random_state=RANDOM_STATE)
        param_grid = {p: v if isinstance(v, list) else [
            v] for p, v in param_grid.items()}
...
def main(signals, features, Y, esp_ids, config):
    # raw data for deep neural networks.
    X = np.expand_dims(signals[:, :6100], axis=1)
    ...
    for classifier_name, classifier in combineTransformerClassifier(fe_list, base_classifiers):
        ...
        Results[classifier_name] = cross_validate(classifier, X,
                                                  labels, groups=esp_ids, scoring=scoring, cv=sampler)
    ...
    if (config.train_single_classifiers):
        ...
        X = features[ICTAI2016_features].values.astype(np.float32)
            ...
            for rname, rs in result.items():
                if (rname.startswith('test_') or 'time' in rname):
                    ...
                    for i, r in enumerate(rs):
                        results_asmatrix.append(
                            (classif_name, metric_name, i+1, r))
        df = pd.DataFrame(results_asmatrix, columns=[
                          'classifier name', 'metric name', 'fold id', 'value'])
...
if __name__ == '__main__':
    ...
    print("Loading data...")
    data = np.load('%s/spectrum.npz' % args.inputdata)

    features = pd.read_csv('%s/features.csv' %
                           args.inputdata, delimiter=';')

    # The first 100 data points are usually just noise.
    signals = np.array([data[str(i)] for i in features.id], dtype=np.float32)
    signals = signals[:, 100:]

    labels = features['label'].map({
        'Normal': 0,
        'Rubbing': 1,
        'Faulty sensor': 2,
        'Misalignment': 3,
