...
class MLP_module(nn.Module):
    def __init__(self, num_inputs, num_outputs):
        ...
        self.fc = nn.Sequential(nn.Linear(num_inputs, hidden_layer_size), nn.LeakyReLU(negative_slope=0.05),
                                nn.Linear(hidden_layer_size, hidden_layer_size), nn.LeakyReLU(
                                    negative_slope=0.05),
...
class TensorBoardCallback(skorch.callbacks.TensorBoard):
    def on_train_end(self, net, X=None, y=None, **kwargs):
        neural_net_params = {k: v for k, v in net.get_params(
        ).items() if type(v) in (int, float, str, bool)}
...
class NeuralNetBase(NeuralNet):
    ...
    def get_default_callbacks(self):
        ...
        checkpoint_callback = skorch.callbacks.Checkpoint(dirname=self.cache_dir,
                                                          ...
                                                          monitor=self.monitor_loss + '_best',
        ...
        callbacks += [checkpoint_callback,
                      LoadEndState(checkpoint_callback, delete_checkpoint=True)]

        callbacks.append(skorch.callbacks.EarlyStopping(
            monitor=self.monitor_loss, patience=100, threshold=1e-4))
...
class NeuralNetTransformer(NeuralNetBase, TransformerMixin):
    def __init__(self, module, init_random_state=None, validation_dataset=None, *args, **kwargs):
        super().__init__(module, *args, init_random_state=init_random_state,
                         validation_dataset=validation_dataset, **kwargs)
    ...
    def encode(self, x):
        with torch.no_grad():
            if (len(x.shape) == 1):
                ...
                n = x.shape[0]
                ret = torch.empty((n, self.num_outputs),
                                  dtype=torch.float32).cuda()
                k = 0
                for i in range(0, n, 8):
                    batch = torch.tensor(
                        x[i:i+8, :6100], dtype=torch.float32).cuda()
...
class MyNeuralNetClassifier(skorch.NeuralNetClassifier, NeuralNetBase, TransformerMixin):
    ...
    def __init__(self, module__embedding_net, module__n_classes, init_random_state,
        ...
        super().__init__(module=module, module__embedding_net=module__embedding_net,
                         module__n_classes=module__n_classes, **kwargs)
        self.init_random_state = init_random_state
...
class MLPClassifier(skorch.NeuralNetClassifier, NeuralNetBase):
    def __init__(self, num_inputs, num_outputs, module=MLP_module, device='cuda',
                 train_split=skorch.dataset.CVSplit(
                     0.25, stratified=True, random_state=0),
        ...
        optimizer_parameters = {
            "optimizer__"+key: v for key, v in optimizer_parameters.items()}
